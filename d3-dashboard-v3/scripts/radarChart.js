// RadarChart function to create a radar chart visualization
function RadarChart(id, data, options) {
    ...
    // Overwrite default options with provided options
    ...
    // Get the names of each axis, total number of axes, and radius of the outermost circle
    ...
    // Remove any existing chart with the same id
    ...
    // Append a g element and translate it
    var g = svg.append("g")
    ...
    // Use a cardinal-closed curve if roundStrokes is true
    if (cfg.roundStrokes) {
    ...
    // Text indicating at what percentage each level is
    axisGrid.selectAll(".axisLabel")
        ...
        .attr("fill", "#737373")
        .style("fill", "#ffffff") // Color the percentage text white
    ...
    axes.append("text")
        .style("fill", "#ffffff") // Color the axis labels white
        .attr("class", "legend")
        .style("font-size", "11px")
        .attr("text-anchor", "middle")
        .attr("dy", (d) => {
            // Adjust the dy value for "Unemployment Rate" axis
        ...
        .attr("x", (d, i) => {
            // Add margin left for "Unemployment Rate" axis
            const marginLeft = d === "Unemployment Rate" ? -35 : 0;
            // Add margin right for "Life Expectancy" axis
            const marginRight = d === "Life Expectancy" ? -20 : 0;
            return rScale(maxValue * cfg.labelFactor) * Math.cos(angleSlice * i - Math.PI / 2) + marginLeft - marginRight;
        })
        .attr("y", (d, i) => {
            // Add margin down for "Unemployment Rate" axis
            const marginBottom = d === "Unemployment Rate" ? -25 : 0;
            return rScale(maxValue * cfg.labelFactor) * Math.sin(angleSlice * i - Math.PI / 2) + marginBottom;
        })
        .text(d => d)
        .call(wrap, cfg.wrapWidth); // Call the wrap function to wrap long axis labels
    ...
    blobs.append("path")
        ...
        .on('mouseover', function (event, d) {
            // On mouseover, reduce the opacity of all radar areas except the current one
       ...
       .on('mouseout', function () {
           // On mouseout, reset the opacity of all radar areas
           d3.selectAll(".radarArea")
               .transition().duration(200)
               .style("fill-opacity", cfg.opacityArea);
       });
...
// Set margin and dimensions for the chart
...
// Load data from CSV file and process it
